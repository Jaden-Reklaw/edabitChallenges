/*
Transcribe To mRNA
Transcribe the given DNA strand into corresponding 
mRNA - a type of RNA, that will be formed from DNA 
after transcription. DNA has the bases A, T, G and C, 
while RNA converts to U, A, C and G respectively.

Examples
dnaToRna("ATTAGCGCGATATACGCGTAC") ➞ "UAAUCGCGCUAUAUGCGCAUG"

dnaToRna("CGATATA") ➞ "GCUAUAU"

dnaToRna("GTCATACGACGTA") ➞ "CAGUAUGCUGCAU"
Notes
Transcription is the process of making complementary strand.
A, T, G and C in DNA converts to U, A, C and G respectively in mRNA.
*/

//My Answer
const dnaToRna = dna => dna.split('').map(letter => letter === 'A' ? 'U': letter === 'T' ? 'A' : letter === 'G' ? 'C' : 'G').join('');

//Smart way using objects
function dnaToRna(dna) {
    const converted = {'A': 'U', 'T': 'A', 'C': 'G', 'G': 'C'};
    return [...dna].map(i => converted[i]).join('');
}
